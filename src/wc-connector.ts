...
export class WCConnector implements IConnector {
    ...
    constructor(meta?: DAppMetadata) {
        ...
        this.dAppConnector.init([WalletUpdatedEvent, WalletLockedEvent], ['handshake']);
    }
    ...
    public call<RequestT, ResponseT, OutputT>(executable: Executable<RequestT, ResponseT, OutputT>): Promise<OutputT> {
    ...
    public checkTransaction<T extends Transaction>(transaction: T): Promise<T> {
        if (!this.activeSigner) {
            throw noSessionError();
        }
    ...
    public populateTransaction<T extends Transaction>(transaction: T): Promise<T> {
        if (!this.activeSigner) {
            throw noSessionError();
        }
    ...
    public signTransaction<T extends Transaction>(transaction: T): Promise<T> {
        if (!this.activeSigner) {
            throw noSessionError();
        }
    ...
    public sign(messages: Uint8Array[], signOptions?: KeyPairSignOptions): Promise<SignerSignature[]> {
        if (!this.activeSigner) {
            throw noSessionError();
        }
